...
if __name__ == '__main__':
    ...
    #用于设置程序随机数生成器种子的函数调用，以确保在不同运行中获得可复现的随机数序列
    ...
    if opt.mode=="train" :
        #验证数据集 valid_dataset，这是用于在训练期间验证模型性能的数据集。
        ...
        #创建训练数据加载器 train_loader 和验证数据加载器 valid_loader，用于将数据批次送入模型进行训练和验证
        ...
        scheduler = lambda optimizer: torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40,100], gamma=0.33)

        #创建一个 Trainer对象，用于执行训练任务，指定训练参数、模型、工作目录、优化器、损失函数等。
        ...
        #开始模型训练，设置训练数据加载器、验证数据加载器、训练的最大迭代次数等。
